...
{
    public class TASKSCHEME_PRIVATE_ORBIT
    {

        //数据库连接字符串(web.config来配置)，可以动态更改connectionString支持多数据库.		
        public static string connectionString;
        public TASKSCHEME_PRIVATE_ORBIT()
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT COUNT(1) FROM LHF.TASKSCHEME_PRIVATE_ORBIT");
        ...
        public Model.TASKSCHEME_PRIVATE_ORBIT GetModel(decimal SAT_ID,decimal schemeid)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT * FROM LHF.TASKSCHEME_PRIVATE_ORBIT ");
            strSql.Append(" WHERE SAT_ID=" + SAT_ID);
        ...
        /// 获取泛型数据列表
        /// </summary>
        public List<Model.TASKSCHEME_PRIVATE_ORBIT> GetList()
        {
            StringBuilder strSql = new StringBuilder("SELECT * FROM LHF.TASKSCHEME_PRIVATE_ORBIT");
            using (SqlDataReader dr = DbHelperSQL.ExecuteReader(strSql.ToString()))
            {
                List<Model.TASKSCHEME_PRIVATE_ORBIT> lst = GetList(dr);
                return lst;
            }
        }
        ...
        {
            StringBuilder strSql = new StringBuilder("SELECT DISTINCT SAT_ORBITDATE FROM LHF.TASKSCHEME_PRIVATE_ORBIT");
        ...
        {
            return DbHelperSQL.GetCount("LHF.TASKSCHEME_PRIVATE_ORBIT", condition);
        }
        ...
        /// </summary>
        private List<Model.TASKSCHEME_PRIVATE_ORBIT> GetList(DbDataReader dr)
        {
            List<Model.TASKSCHEME_PRIVATE_ORBIT> lst = new List<Model.TASKSCHEME_PRIVATE_ORBIT>();
            while (dr.Read())
            {
                lst.Add(GetModel(dr));
            }
