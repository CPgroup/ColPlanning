...
namespace CoScheduling.Main.SPYCAM_RANGE
{
    public partial class SENSOR2Modify : Form
    {
        ...
        public void bindComboBoxPLATFORM()
        {
            ...
            ds = dal_spycam_range.GetListDataSet("");
            string strItem = "";
            string strValue = "";
            List<ListItem> items = new List<ListItem>();
            for (int i = 0; i < ds.Tables["SPYCAM_RANGE"].Rows.Count; i++)
            {
                strItem = ds.Tables["SPYCAM_RANGE"].Rows[i]["PLATFORM_Name"].ToString();
                strValue = ds.Tables["SPYCAM_RANGE"].Rows[i]["PLATFORM_ID"].ToString();
