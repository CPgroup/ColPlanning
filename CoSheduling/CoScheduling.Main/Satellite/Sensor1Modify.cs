...
namespace CoScheduling.Main.Satellite
{
    public partial class Sensor1Modify : Form
    {
        ...
        public void bindComboBoxPLATFORM()
        {
            ...
            for (int i = 0; i < ds.Tables["SATELLITE_RANGE"].Rows.Count; i++)
            {
                strItem = ds.Tables["SATELLITE_RANGE"].Rows[i]["PLATFORM_Name"].ToString();
                strValue = ds.Tables["SATELLITE_RANGE"].Rows[i]["PLATFORM_ID"].ToString();
