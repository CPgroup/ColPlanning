...
{
    //无人机观测资源的管理窗口
    public partial class UAVManage : Form
    {
        public UAVManage()
        {
            InitializeComponent();
        ...
        /// <returns></returns>
        public DataSet GetUAVRangeDataSet(string strWhere)
        {
        ...
        /// <summary>
        /// 给dataGridViewUAV绑定无人机信息数据
        /// </summary>
        /// <param name="strWhere">条件</param>
        public void bindUAVInfo(string strWhere)
        {
            dataGridViewUAV.AutoGenerateColumns = false;
            this.dataGridViewUAV.DataSource = GetUAVRangeDataSet(strWhere).Tables["UAV_RANGE"];
        }
        /// <summary>
        ...
        private void UAVManage_Load(object sender, EventArgs e)
        {
        ...
        #region 无人机信息按钮操作
        //无人机添加
        private void ButtonUAVAdd_Click(object sender, EventArgs e)
        {
            UAV.UAVAdd newform = new UAV.UAVAdd();
        ...
        private void ButtonUAVModify_Click(object sender, EventArgs e)
        {
            string uav_id = this.dataGridViewUAV.CurrentRow.Cells[0].Value.ToString();
            TaskRequirement.UAVModify newform = new TaskRequirement.UAVModify(uav_id);
            newform.StartPosition = FormStartPosition.CenterScreen;
            //子窗体关闭，刷新卫星列表
            if (newform.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void ButtonUAVDelete_Click(object sender, EventArgs e)
        {
            string platform_id = this.dataGridViewUAV.CurrentRow.Cells[0].Value.ToString();
            if (MessageBox.Show("确定删除任务记录?此删除不可恢复！", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    dal_uav_range.Delete(Convert.ToDecimal(platform_id));
        ...
        private void ButtonSensorAdd_Click(object sender, EventArgs e)
        {
            ...
            UAV.Sensor1Add newform = new UAV.Sensor1Add(platform_id);
            newform.StartPosition = FormStartPosition.CenterScreen;
            //子窗体关闭，刷新卫星列表
            if (newform.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void ButtonSensorModify_Click(object sender, EventArgs e)
        {
            string platform_id = this.dataGridViewUAV.CurrentRow.Cells[0].Value.ToString();
            string sensor_id = this.dataGridViewSensor.CurrentRow.Cells[0].Value.ToString();
            UAV.Sensor1Modify newform = new UAV.Sensor1Modify(sensor_id);
            newform.StartPosition = FormStartPosition.CenterScreen;
            //子窗体关闭，刷新卫星列表
            if (newform.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void ButtonBandAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                platform_id = this.dataGridViewUAV.CurrentRow.Cells[0].Value.ToString();
            }
            catch (System.Exception ex)
            {
            ...
            //生成窗体进行添加
            UAV.BandAdd newform = new UAV.BandAdd(sensor_id);
            newform.StartPosition = FormStartPosition.CenterScreen;
            //子窗体关闭，刷新卫星列表
            if (newform.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
        ...
        private void ButtonBandModify_Click(object sender, EventArgs e)
        {
            ...
            UAV.BandModify newform = new UAV.BandModify(band_id, platform_id, sensor_id);
            newform.StartPosition = FormStartPosition.CenterScreen;
            //子窗体关闭，刷新卫星列表
            if (newform.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
        ...
        #region 表格单元点击操作
        private void dataGridViewUAV_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            string platform_id;
            platform_id = this.dataGridViewUAV.CurrentRow.Cells[0].Value.ToString();
            //显示无人机载荷信息，根据无人机ID
