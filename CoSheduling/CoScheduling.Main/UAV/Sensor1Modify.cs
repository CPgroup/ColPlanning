...
namespace CoScheduling.Main.UAV
{
    public partial class Sensor1Modify : Form
    {
        ...
        public void bindComboBoxPLATFORM()
        {
            ...
            ds = dal_uav_range.GetListDataSet("");
            string strItem = "";
            string strValue = "";
            List<ListItem> items = new List<ListItem>();
            for (int i = 0; i < ds.Tables["UAV_RANGE"].Rows.Count; i++)
            {
                strItem = ds.Tables["UAV_RANGE"].Rows[i]["PLATFORM_Name"].ToString();
                strValue = ds.Tables["UAV_RANGE"].Rows[i]["PLATFORM_ID"].ToString();
